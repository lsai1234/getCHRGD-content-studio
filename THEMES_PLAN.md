...
| D17 | **The Multiverse is not a reality format** — no villa, no couplings or recouplings, no public vote, no eliminations, no ceremony. A persistent world, not a season of a show. Supersedes the Love-Island-shaped half of D1/D6 | `Season` defaults to nothing and the retired "The Villa" default is dropped from any stored canon on read, so it stops being the first line of every brief. The ban is written into the show voice, the story prompt and `config/story_process.md`. A season remains available as a deliberate operator choice. |
...
| D15 | **Roster locked** — Orangina in (half woman, half orange, cheating on her orange husband — corrected from an invented 'wellness girl' version), her husband Orangino added so the gag has somebody to walk in, Clavicular dropped, Bonnie Blue left out | `MULTIVERSE_ROSTER.md` is final; it becomes `config/roster.toml` in Phase 3. |
