...
>
> **Revision 3 (shipped) — no door skips it, and you can watch it happen.** The gate was only running on one of the paths that reach a paid slide-1 image. Two others didn't: `webapp.api_render` (the sync `/api/render/{id}` endpoint) called `render_carousel` directly, and `worker._handle_render_slide` regenerated slide 1 with no concept check at all — which meant the sharper hook the *scroll test* invents went straight to a paid image having never faced the judge. Both now route through the single `services.ensure_concept_gated`, alongside `render_idea` (create journey, review screen, CLI, orchestrator). What makes that safe to apply everywhere is a **concept fingerprint** (sha1 of slide 1's headline + supporting + visual brief) stamped with the verdict: "regenerate the background" on an approved opener matches the fingerprint and reuses the verdict — new pixels, no surprise rewrite, no second tournament — while a hand-edited hook or one the scroll test wrote doesn't match, and gets checked properly. `force=True` re-runs it on demand. **Visibility:** the gate now emits a structured stage log (`GATE_STAGES` — the four keys and their labels) through an `on_stage` channel alongside the existing prose notes. `_gate_reporters` writes both onto the render job, `api_idea_detail` exposes `active_jobs.*.gate_stages`, and the create journey renders a live checklist above the image progress bar — each check lighting up as it runs with its result inline ("4 rival openers on the table", "a sharper opener won: …", "scored 9/9", "at a glance they got: …"). The stage log is persisted with the verdict, and the write step says up front what slide 1 is about to go through.
