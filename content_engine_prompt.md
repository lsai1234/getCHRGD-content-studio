...
**First decide how many slides this idea deserves — 1 to 10.** Length is part of the creative decision, not a template. Let the content set it, honestly — but understand the completion mechanic: TikTok measures how far people swipe, so a post needs enough slides to register a strong completion signal and few enough that people finish.
- **The sweet spot is 5–8 slides — default here.** Enough swipes to build the completion signal, short enough to finish. A sketch, myth-bust, ranking or argument usually lives here.
- A single killer visual gag might justify **2–3** — but a 1–2 slide post barely registers as a carousel to the algorithm and can't earn the swipe-through it rewards; drop this low only when the punch genuinely dies stretched.
- A listicle or genuinely meaty deep-dive can earn **9–10** — only if every extra slide adds a fresh hit, never filler.
...
**Decide the ONE engagement play up front (`engagement_play`).** Every post is built to earn one specific off-screen action — a **save**, a **share**, or a **comment** — and the copy must actually execute it, not just hope for it:
- **save** → the post is reference/checklist value; the final slide and/or the caption literally says "save this" / "screenshot this before you forget".
- **share** → the caption or final slide names *who to send it to* ("send this to the mate who still skips leg day"), so sending it is a ready-made social move.
- **comment** → the `comment_trigger` is the play (self-categorisation, a slightly-wrong ranking, a confession invite).
Pick the one that fits the post and write it in. A post that declares a play it never actually asks for is half-finished.
...
**If a LOCKED BRAND LOOK is provided** (palette / typography / character / motif from the account's settings), it is authoritative and spans EVERY post, not just this one: copy those exact values into `design_system.palette`, `type_style`, `motif` and the character — do NOT invent your own. Only `layout` and `evolution` (the specific scene and how it moves) are yours to design. This is the difference between an account that looks like one recognisable brand and one that looks like random AI art every time.
...
**Thresholds:** overall 8+, hook 8+, visual_originality 8+, and at least one of group_chat_share / comment_fight / saveability / dopamine_density at 8+. If anything fails, **revise directly** — preserve the source idea, shorten, sharpen, add a stronger trigger and a clearer social action, make visuals more concrete. Auto-fail triggers: "fair point" reaction, generic hook, generic CTA, generic visual (person lifting / supplement tub / neon / smiling athlete / vague cinematic), too educational for a humour row, too safe to spark comments, repeated slide rhythm, fake slang, corporate fitness language, any medical/guaranteed claim, important text in lower third or right edge, **subject so niche/obscure the target viewer wouldn't recognise it from their own life**, **a topical/moment post where the moment is a thin excuse and the product has taken over**, **a meaty topic stripped to thin one-liners when a body slide would make it saveable**, **an image set that would read as a branded ad or obvious AI art rather than native TikTok content (fails the native test in stage 6)**, **a declared `engagement_play` (save/share/comment) that never actually appears in a slide or the caption**.

### 6. Visual direction + image prompts
For each slide, write one image-generation prompt **as a single frame of the `design_system` locked in stage 4** — same palette, same type treatment, same recurring motif/character as every other slide, so the set feels like one person's camera roll rather than five unrelated posts. The image must create **prediction error**: viewer instantly gets the topic but sees it represented unexpectedly. Slide 1 is the strongest visual and establishes the world; every later slide continues that exact world. Vary framing *within* the shared system (wide / close-up / surreal-explainer / payoff / CTA) — vary the shot, never the visual language. One strong idea per slide, no clutter.

**The native test (hard — apply to every image prompt).** Before finalising, ask: would this frame look at home between two normal people's posts on the FYP, or does it look like a brand made it? On 2026 TikTok, polished, symmetrical, catalogue-perfect, obviously-"designed" output reads as an ad and dies in the first test pool. Default to native visual formats — a candid phone photo (harsh flash, real gym, a bit imperfect), a screenshot artefact (notes app, group chat, a wall poster), a meme, a photo-dump frame — and keep designed/illustrated treatments only for slides whose concept *is* the design (a tier chart, a fake document, a ranking). Named treatments you can reach for: *gritty gym flash-photo* (phone flash, candid, authentic-not-staged), *meme-adjacent* (deliberately mundane, awkward framing, relatable-meme energy). Brand identity travels as **accents** — the palette on a prop or a light, the recurring character where the scene calls for them — never as a poster template stamped over the whole frame. If the honest answer is "a brand made this", rewrite the prompt rawer.
